...
- **Region**: us-east-1
- **URL**: [https://tfatikeupkydferbcsbc.supabase.co](https://tfatikeupkydferbcsbc.supabase.co)
...
- **StrongBox on-chain:** deploy y `deposit` los firma el **frontend** (MetaMask). API: `POST /api/strongbox/confirm-deploy`, `POST /api/strongbox/confirm-deposit`; `GET /api/caja-fuerte/balance` usa BNB real si está deployada (`RPC_URL` + ethers).
...
## 2026-03-29 — API: integración on-chain (sin deploy mock)

- **`api/src/services/chainProvider.ts`**: `JsonRpcProvider` con `RPC_URL`.
- **`confirmDeploy`**: `getCode` + update `caja_fuerte` (`contract_address`, `is_deployed`, `deploy_tx_hash`).
- **`confirmDeposit`**: valida receipt, `value`, `from`/wallet del usuario; insert en `transactions`.
- **Rutas**: `POST /api/strongbox/confirm-deploy`, `POST /api/strongbox/confirm-deposit` (reemplaza el antiguo `deposit` mock).
- **Balance caja fuerte**: `source: "rpc"` + `getBalance` cuando hay contrato deployado; si no, mock.
- Documentación: `docs/API.md`, `api/http/api.http`, entrada anterior en este log actualizada.
