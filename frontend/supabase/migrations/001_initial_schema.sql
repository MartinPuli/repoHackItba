...
CREATE TYPE tx_type AS ENUM ('deposit', 'withdraw', 'send', 'swap', 'yield_deposit', 'yield_withdraw', 'bridge', 'off_ramp');
CREATE TYPE tx_status AS ENUM ('pending', 'confirmed', 'failed', 'reverted');
CREATE TYPE agent_action_type AS ENUM ('analysis', 'suggestion', 'prepare_tx', 'execute_tx', 'compliance_check', 'rebalance', 'yield_optimize', 'reset_deadman', 'alert');
...
CREATE TABLE caja_fuerte (
  ...
  dead_man_timeout_seconds INTEGER NOT NULL DEFAULT 7776000, -- 90 days
...
ALTER TABLE transactions ADD CONSTRAINT fk_agent_decision
...
CREATE TRIGGER tr_caja_fuerte_updated BEFORE UPDATE ON caja_fuerte FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE TRIGGER tr_herederos_updated BEFORE UPDATE ON herederos FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE TRIGGER tr_yield_positions_updated BEFORE UPDATE ON yield_positions FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
RETURNS void AS $$
BEGIN
  UPDATE session_keys SET status = 'expired' WHERE status = 'active' AND expires_at < NOW();
END;
$$ LANGUAGE plpgsql;
...
SELECT
  ...
  cf.recovery_state,
  (SELECT COUNT(*) FROM session_keys sk WHERE sk.user_id = u.id AND sk.status = 'active') AS active_session_keys,
  (SELECT COUNT(*) FROM alerts a WHERE a.user_id = u.id AND a.is_read = false) AS unread_alerts,
  (SELECT COALESCE(SUM(yp.amount_usd), 0) FROM yield_positions yp WHERE yp.user_id = u.id AND yp.is_active = true) AS total_yield_usd
FROM users u
